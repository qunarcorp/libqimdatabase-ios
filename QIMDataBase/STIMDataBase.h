...
#define STIMDBAutorelease(__v) ([__v autorelease]);
#define STIMDBReturnAutoreleased STIMDBAutorelease

#define STIMDBRetain(__v) ([__v retain]);
#define STIMDBReturnRetained STIMDBRetain

#define STIMDBRelease(__v) ([__v release]);

#define STIMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
// -fobjc-arc
#define STIMDBAutorelease(__v)
#define STIMDBReturnAutoreleased(__v) (__v)

#define STIMDBRetain(__v)
#define STIMDBReturnRetained(__v) (__v)

#define STIMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details.
#if OS_OBJECT_USE_OBJC
#define STIMDBDispatchQueueRelease(__v)
#else
#define STIMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#endif
#endif
...
typedef int(^STIMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

typedef NS_ENUM(int, STIMDBCheckpointMode) {
    STIMDBCheckpointModePassive = 0, // SQLITE_CHECKPOINT_PASSIVE,
    STIMDBCheckpointModeFull = 1, // SQLITE_CHECKPOINT_FULL,
    STIMDBCheckpointModeRestart = 2, // SQLITE_CHECKPOINT_RESTART,
    STIMDBCheckpointModeTruncate = 3  // SQLITE_CHECKPOINT_TRUNCATE
};

/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in STIMDB are:
 
 - `STIMDataBase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<DataReader>` - Represents the results of executing a query on an `STIMDataBase`.
 - `<STIMDataBaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.
 
 ### See also
 
 - `<STIMDataBasePool>` - A pool of `STIMDataBase` objects.
 - `<STIMDBStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [STIMDB on GitHub](https://github.com/ccgus/STIMDB) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [STIMDB mailing list](http://groups.google.com/group/STIMDB)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `STIMDataBase` object and use it across multiple threads. Instead, use `<STIMDataBaseQueue>`.
...
@interface STIMDataBase : NSObject
...
/** Create a `STIMDataBase` object.
 
 An `STIMDataBase` is created with a path to a SQLite database file.  This path can be one of these three:
 
 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `STIMDataBase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `STIMDataBase` connection is closed.
 
 For example, to create/open a database in your Mac OS X `tmp` folder:
 
 STIMDataBase*db = [STIMDataBasedatabaseWithPath:@"/tmp/tmp.db"];
 
 Or, in iOS, you might open a database in the app's `Documents` directory:
 
 NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
 NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
 STIMDataBase*db     = [STIMDataBasedatabaseWithPath:dbPath];
 ...
 @return `STIMDataBase` object if successful; `nil` if failure.
...
/** Create a `STIMDataBase` object.
 ...
 2. `nil`.  An in-memory database is created.  This database will be destroyed with the `STIMDataBase` connection is closed.
...
/** Initialize a `STIMDataBase` object.
...
/** Initialize a `STIMDataBase` object.
 ...
 An `STIMDataBase` is created with a local file URL to a SQLite database file.  This path can be one of these three:
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) STIMDBExecuteStatementsCallbackBlock _Nullable)block;
 ...
 */
- (BOOL)checkpoint:(STIMDBCheckpointMode)checkpointMode error:(NSError *_Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return YES on success, otherwise NO.
 */
- (BOOL)checkpoint:(STIMDBCheckpointMode)checkpointMode name:(NSString *_Nullable)name error:(NSError *_Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return YES on success, otherwise NO.
 */
- (BOOL)checkpoint:(STIMDBCheckpointMode)checkpointMode name:(NSString *_Nullable)name logFrameCount:(int *_Nullable)logFrameCount checkpointCount:(int *_Nullable)checkpointCount error:(NSError *_Nullable *)error;
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 myDB.dateFormat = [STIMDataBasestoreableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one STIMDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using STIMDataBase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to STIMDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using STIMDB you will not need to interact directly with `STIMDBStatement`, but rather with `<STIMDataBase>` and `<DataReader>` only.
...
@interface STIMDBStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
