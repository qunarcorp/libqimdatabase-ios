...
#pragma mark STIMDataBaseinstantiation and deallocation
...
+ (instancetype)databaseWithURL:(NSURL *)url {
    return STIMDBReturnAutoreleased([[self alloc] initWithURL:url]);
}
...
- (void)dealloc {
    [self close];
    STIMDBRelease(_openResultSets);
    STIMDBRelease(_cachedStatements);
    STIMDBRelease(_dateFormat);
    STIMDBRelease(_databasePath);
    STIMDBRelease(_openFunctions);
...
+ (NSString *)STIMDBUserVersion {
    return @"2.7.5";
}

// returns 0x0240 for version 2.4.  This makes it super easy to do things like:
// /* need to make sure to do X with STIMDB version 2.4 or later */
// if ([STIMDataBaseSTIMDBVersion] >= 0x0240) { … }

+ (SInt32)STIMDBVersion {

    // we go through these hoops so that we only have to change the version number in a single spot.
    static dispatch_once_t once;
    static SInt32 STIMDBVersionVal = 0;

    dispatch_once(&once, ^{
        NSString *prodVersion = [self STIMDBUserVersion];
        ...
        char *e = nil;
        STIMDBVersionVal = (int) strtoul([junk UTF8String], &e, 16);

    });

    return STIMDBVersionVal;
}
...
static int STIMDBDatabaseBusyHandler(void *f, int count) {
    STIMDataBase *self = (__bridge STIMDataBase *) f;
...
- (void)setMaxBusyRetryTimeInterval:(NSTimeInterval)timeout {
    ...
    if (timeout > 0) {
        sqlite3_busy_handler(_db, &STIMDBDatabaseBusyHandler, (__bridge void *) (self));
    } else {
...
// but for folks who don't bother noticing that the interface to STIMDataBasechanged,
// we'll still implement the method so they don't get suprise crashes
- (int)busyRetryTimeout {
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"STIMDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"STIMDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (void)closeOpenResultSets {
    ...
    NSSet *openSetCopy = STIMDBReturnAutoreleased([_openResultSets copy]);
...
- (void)clearCachedStatements {
    ...
    for (NSMutableSet *statements in [_cachedStatements objectEnumerator]) {
        for (STIMDBStatement *statement in [statements allObjects]) {
            [statement close];
        }
    }
...
- (STIMDBStatement *)cachedStatementForQuery:(NSString *)query {

    NSMutableSet *statements = [_cachedStatements objectForKey:query];

    return [[statements objectsPassingTest:^BOOL(STIMDBStatement *statement, BOOL *stop) {
...
- (void)setCachedStatement:(STIMDBStatement *)statement forQuery:(NSString *)query {
    NSParameterAssert(query);
    if (!query) {
        NSLog(@"API misuse, -[STIMDataBasesetCachedStatement:forQuery:] query must not be nil");
        return;
    }
...
+ (NSDateFormatter *)storeableDateFormat:(NSString *)format {
    ...
    NSDateFormatter *result = STIMDBReturnAutoreleased([[NSDateFormatter alloc] init]);
    result.dateFormat = format;
    result.timeZone = [NSTimeZone timeZoneForSecondsFromGMT:0];
    result.locale = STIMDBReturnAutoreleased([[NSLocale alloc] initWithLocaleIdentifier:@"en_US"]);
    return result;
}
...
- (void)setDateFormat:(NSDateFormatter *)format {
    STIMDBAutorelease(_dateFormat);
    _dateFormat = STIMDBReturnRetained(format);
}
...
- (void)warnInUse {
    NSLog(@"The STIMDataBase%@ is currently in use.", self);

#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The STIMDataBase%@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_isOpen) {
        ...
        NSLog(@"The STIMDataBase%@ is not open.", self);

#ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The STIMDataBase%@ is not open.", self);
            abort();
        }
#endif
...
- (NSError *)errorWithMessage:(NSString *)message {
    ...
    return [NSError errorWithDomain:@"STIMDataBase" code:sqlite3_errcode(_db) userInfo:errorMessage];
}
...
- (DataReader *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    STIMDBStatement *statement = 0x00;
    DataReader *rs = 0x00;
    ...
    STIMDBRetain(statement); // to balance the release below

    if (!statement) {
        statement = [[STIMDBStatement alloc] init];
...
- (BOOL)executeUpdate:(NSString *)sql error:(NSError *_Nullable __autoreleasing *)outErr withArgumentsInArray:(NSArray *)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    */
    ...
    int rc = 0x00;
    sqlite3_stmt *pStmt = 0x00;
    STIMDBStatement *cachedStmt = 0x00;
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[STIMDBStatement alloc] init];

        [cachedStmt setStatement:pStmt];

        [self setCachedStatement:cachedStmt forQuery:sql];

        STIMDBRelease(cachedStmt);
    }
...
int STIMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int STIMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) STIMDBExecuteStatementsCallbackBlock)block {

    int rc;
    char *errmsg = nil;

    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? STIMDBExecuteBulkSQLCallback : nil, (__bridge void *) (block), &errmsg);
...
static NSString *STIMDBEscapeSavePointName(NSString *savepointName) {
...
- (BOOL)startSavePointWithName:(NSString *)name error:(NSError *_Nullable __autoreleasing *)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"savepoint '%@';", STIMDBEscapeSavePointName(name)];
...
- (BOOL)releaseSavePointWithName:(NSString *)name error:(NSError *_Nullable __autoreleasing *)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"release savepoint '%@';", STIMDBEscapeSavePointName(name)];
...
- (BOOL)rollbackToSavePointWithName:(NSString *)name error:(NSError *_Nullable __autoreleasing *)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"rollback transaction to savepoint '%@';", STIMDBEscapeSavePointName(name)];
...
- (BOOL)checkpoint:(STIMDBCheckpointMode)checkpointMode error:(NSError *__autoreleasing *)error {
    return [self checkpoint:checkpointMode name:nil logFrameCount:NULL checkpointCount:NULL error:error];
}

- (BOOL)checkpoint:(STIMDBCheckpointMode)checkpointMode name:(NSString *)name error:(NSError *__autoreleasing *)error {
    return [self checkpoint:checkpointMode name:name logFrameCount:NULL checkpointCount:NULL error:error];
}

- (BOOL)checkpoint:(STIMDBCheckpointMode)checkpointMode name:(NSString *)name logFrameCount:(int *)logFrameCount checkpointCount:(int *)checkpointCount error:(NSError *__autoreleasing *)error {
...
void STIMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void STIMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if !__has_feature(objc_arc)
...
- (void)makeFunctionNamed:(NSString *)name arguments:(int)arguments block:(void (^)(void *context, int argc, void **argv))block {
    ...
    id b = STIMDBReturnAutoreleased([block copy]);

    [_openFunctions addObject:b];

    /* I tried adding custom functions to release the block when the connection is destroyed- but they seemed to never be called, so we use _openFunctions to store the values instead. */
#if !__has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], arguments, SQLITE_UTF8, (void*)b, &STIMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], arguments, SQLITE_UTF8, (__bridge void *) b, &STIMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
...
@implementation STIMDBStatement
