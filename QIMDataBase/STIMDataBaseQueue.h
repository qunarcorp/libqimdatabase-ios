...
/** To perform queries and updates on multiple threads, you'll want to use `STIMDataBaseQueue`.
 
 Using a single instance of `<STIMDataBase>` from multiple threads at once is a bad idea.  It has always been OK to make a `<STIMDataBase>` object *per thread*.  Just don't share a single instance across threads, and definitely not across multiple threads at the same time.
 
 Instead, use `STIMDataBaseQueue`. Here's how to use it:
 
 First, make your queue.
 
 STIMDataBaseQueue *queue = [STIMDataBaseQueue databaseQueueWithPath:aPath];
 
 Then use it like so:
 
 [queue inDatabase:^(STIMDataBase*db) {
 ...
 [queue inTransaction:^(STIMDataBase*db, BOOL *rollback) {
 [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:1]];
 [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:2]];
 [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:3]];
 ...
 *rollback = YES;
 ...
 `STIMDataBaseQueue` will run the blocks on a serialized queue (hence the name of the class).  So if you call `STIMDataBaseQueue`'s methods from multiple threads at the same time, they will be executed in the order they are received.  This way queries and updates won't step on each other's toes, and every one is happy.
 
 ### See also
 
 - `<STIMDataBase>`
 
 @warning Do not instantiate a single `<STIMDataBase>` object and use it across multiple threads. Use `STIMDataBaseQueue` instead.
 
 @warning The calls to `STIMDataBaseQueue`'s methods are blocking.  So even though you are passing along blocks, they will **not** be run on another thread.
 
 */

@interface STIMDataBaseQueue : NSObject
/** Path of database */
...
/** Create queue using path.
 ...
 @return The `STIMDataBaseQueue` object. `nil` on error.
 */
...
// If you need to nest, use STIMDataBase's startSavePointWithName:error: instead.
